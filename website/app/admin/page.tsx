...
export default function AdminPage() {
  ...
  const restoreFromLocal = () => {
    if (!confirm("This will overwrite the current Cloud menu with data from your local browser storage. Continue?")) return
    
    if (typeof window !== "undefined") {
      const raw = window.localStorage.getItem("sb-menu-data-v1")
      if (raw) {
        try {
          const localData = JSON.parse(raw)
          // Simple validation/normalization
          if (localData && typeof localData === "object") {
             // We cast this to AppData, effectively relying on MenuProvider to sanitize if needed
             // But MenuProvider only sanitizes on load.
             // We should trust the local backup structure is mostly correct since it was generated by the app.
             setData(localData as any)
             alert("Local data restored! It is now saving to the Cloud...")
             return
          }
        } catch (e) {
          console.error(e)
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="rounded-lg border border-coffee-light/30 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-800 hover:bg-amber-100"
            >
              Restore / Initialize
            </button>
            <button
